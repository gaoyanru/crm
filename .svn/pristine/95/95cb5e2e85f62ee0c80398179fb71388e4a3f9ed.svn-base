angular.module('crmApp').controller('Contract_manage', ['$scope', '$http', '$state', '$uibModal', 'user', '$q',
  ...
  function ($scope, $http, $uibModal, $uibModalInstance, FileUploader, $filter, UserServe, contractItem, projectItems) {
    ...
    $scope.check = function () { // 审单人员审核
      ...
      var url = '/api/contractpass/?contractId=' + $scope.postDetail.OrderId
      $http.post(url, $scope.postDetail).success(function(res) {
        console.log(res)
        if(res.status) {
          $uibModalInstance.close()
        }
    ...
    $scope.ok = function (ev) {
      ...
      var RealName = user.RealName
      if ($scope.remark) {
        $scope.remark = $scope.postDetail.Description + $scope.remark  + '{' + RealName + '}'
      }
      console.log($scope.remark, RealName)
      $scope.postDetail.Description = $scope.remark
  ...
  $scope.save = function() {
    ...
    if ($scope.Remark) {
      $scope.postData.remark = $scope.postData.Description + $scope.Remark + '{' + RealName + '}'
    }
    var post = {}
    post.contractId = $scope.postData.OrderId
    post.remark = $scope.postData.remark
    // console.log(post)
    $http.post('/api/ContractReject', post).success(function(res) {
      // console.log(res)
        if (res.status) {
            $uibModalInstance.close();
        }
