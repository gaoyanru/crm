...
angular.module('crmApp').controller('ContractEndManage', ['$scope', '$http', '$state', 'Excel', '$uibModal', 'user', '$q', function($scope, $http, $state, Excel, $uibModal, user, $q) {
    ...
    $scope.changeDate = function() {
        ...
        // 日期功能结束
}]).controller('MarkRefuse_end', ['$scope', '$http', '$uibModalInstance', 'user','contractMsg','signFrom','title', function($scope, $http, $uibModalInstance, users,contractMsg,signFrom,title) {
    // var contract = item
    var users = users.get()
    $scope.title = title
    $scope.sign = signFrom
...
angular.module('crmApp').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('views/contract_end_manage.html',
    "<div class=\"recharge\"> <div class=\"well well-sm u-search clearfix\"> <form class=\"form-inline\" style=\"position:relative;padding-right:150px\"> <div class=\"form-group\" style=\"margin: 5px 10px 5px 10px\"> <label>序列ID</label>： <input type=\"text\" ng-model=\"search.Id\" class=\"form-control\"> </div> <div class=\"form-group\" style=\"margin: 5px 10px 5px 10px\"> <label>公司名称</label>： <input type=\"text\" ng-model=\"search.companyName\" class=\"form-control\"> </div> <div class=\"form-group\" style=\"margin: 5px 10px 5px 10px\"> <label>公司联系人</label>： <input type=\"text\" ng-model=\"search.contacts\" class=\"form-control\"> </div> <div class=\"form-group\" style=\"margin: 5px 10px 5px 10px\"> <label>销售人员</label>： <input type=\"text\" ng-model=\"search.salesId\" class=\"form-control\"> </div> <div class=\"form-group\" style=\"margin: 5px 10px 5px 10px\"> <label>公司状态</label>： <select ng-model=\"search.companystatus\" class=\"form-control\"> <option value=\"\" ng-selected=\"true\">请选择</option> <option ng-repeat=\"(key,value) in companyStatus\" ng-value=\"key\">{{value}}</option> </select> </div> <!-- <div class=\"form-group\" style=\"margin: 5px 10px 5px 10px;\">\n" +
    "        <label>所属区域</label>：\n" +
    "        <select ng-model=\"search.area\" class=\"form-control\">\n" +
    "          <option value=\"\" ng-selected=\"true\">请选择</option>\n" +
    "          <option ng-repeat=\"item in areas\" value=\"{{item.Id}}\">{{item.areaName}}</option>\n" +
    "        </select>\n" +
    "      </div>\n" +
    ...
    "          <option value=\"\" ng-selected=\"true\">请选择</option>\n" +
    "          <option  ng-repeat=\"(key,value) in outworkStatus\" ng-value=\"key\">{{value}}</option>\n" +
    "        </select>\n" +
    "      </div>\n" +
    "      <div class=\"form-group\" style=\"margin: 5px 10px 5px 10px;\">\n" +
    "        <label>会计处理状态</label>：\n" +
    ...
    "        </select>\n" +
    "      </div> --> <div class=\"form-group\" style=\"margin: 5px 10px 5px 10px\"> <label>合同结束期</label>： <select ng-model=\"search.outworkstatus\" class=\"form-control\"> <option value=\"\" ng-selected=\"true\">请选择</option> <option value=\"\">近一个月</option> <option value=\"\">近两个月</option> <option value=\"\">近三个月</option> </select> </div> <div class=\"form-group\" style=\"margin: 5px 10px 5px 10px;min-width:300px\"> <lable style=\"margin-bottom: 5px;font-weight: bold;float:left;line-height:34px;height:34px\">合同签订日期：</lable> <div class=\"row\" style=\"float:left\"> <div class=\"col-md-6\"> <p class=\"input-group\" style=\"margin:0\"> <input type=\"text\" class=\"form-control\" uib-datepicker-popup=\"{{format}}\" ng-model=\"dt1\" is-open=\"popup1.opened\" datepicker-options=\"dateOptions\" close-text=\"Close\" alt-input-formats=\"altInputFormats\" ng-change=\"changeDate()\" id=\"date1\" style=\"padding:6px\"> <span class=\"input-group-btn\"> <button type=\"button\" class=\"btn btn-default\" ng-click=\"open1()\"><i class=\"glyphicon glyphicon-calendar\"></i></button> </span> </p> </div> <div class=\"col-md-6\"> <p class=\"input-group\" style=\"margin:0\"> <input type=\"text\" class=\"form-control\" uib-datepicker-popup ng-model=\"dt2\" is-open=\"popup2.opened\" datepicker-options=\"dateOptions\" close-text=\"Close\" ng-change=\"changeDate()\" id=\"date2\" style=\"padding:6px\"> <span class=\"input-group-btn\"> <button type=\"button\" class=\"btn btn-default\" ng-click=\"open2()\"><i class=\"glyphicon glyphicon-calendar\"></i></button> </span> </p> </div> </div> </div> <button type=\"button\" class=\"btn btn-primary\" ng-click=\"searchFn()\" style=\"position:absolute;right:20px;top:5px\">搜索</button> <!-- <button class=\"btn btn-primary pull-right\" ng-click=\"toExcel()\" style=\"position:absolute;right:20px;top:50px\">导出</button> --> </form> </div> <table class=\"table table-striped table-hover table-bordered\"> <thead> <tr class=\"info\"> <th>序列ID</th> <th>公司名称</th> <th>所属区域</th> <th>公司联系人</th> <th>当前合同编号</th> <th>销售人员</th> <th>合同签订日期</th> <th>服务开始</th> <th>服务结束</th> <th>公司状态</th> <!-- <th>外勤处理状态</th>\n" +
    "          <th>会计处理状态</th> --> <th>操作</th> </tr> </thead> <tbody> <tr ng-repeat=\"customer in customers\"> <td style=\"text-align:center;width: 140px;vertical-align: middle\"> <div ng-show=\"customer.tagStatus\" class=\"mark-style fl\" ng-style=\"{{customer.markBg}}\">{{customer.tagStatus | CustomerMarkStyle}}</div> <div class=\"fr\">{{customer.Id}}</div> </td> <td style=\"text-align:center\">{{customer.CompanyName}}</td> <td style=\"text-align:center\">{{customer.AreaName}}</td> <td style=\"text-align:center\">{{customer.Connector}}</td> <td style=\"text-align:center\">{{customer.ContractNo}}</td> <td style=\"text-align:center\">{{customer.Sales}}</td> <td style=\"text-align:center\">{{customer.ContractDate|tDate}}</td> <td style=\"text-align:center\">{{customer.ServiceStart|tDate}}</td> <td style=\"text-align:center\">{{customer.ServiceEnd|tDate}}</td> <td style=\"text-align:center\">{{customer.ServiceStatus }}</td> <!-- <td style=\"text-align:center\">{{customer.outworkstatus | CustomerOutworkStatus}}</td>\n" +
    "        <td style=\"text-align:center\">{{customer.accountstatus | CustomeraccountStatus}}</td> --> <td class=\"table-opt\" style=\"text-align:center;width:200px\"> <button class=\"btn btn-link\" href=\"javascript:;\" ng-click=\"detail(customer)\">查看</button> <!-- 点击标记 标记按钮不显示 显示取消 --> <button ng-if=\"!customer.tagStatus\" class=\"btn btn-link\" href=\"javascript:;\" ng-click=\"mark(customer)\">标记</button> <button ng-if=\"customer.tagStatus\" class=\"btn btn-link\" href=\"javascript:;\" ng-click=\"cancelmark(customer)\">取消</button> <button class=\"btn btn-link\" href=\"javascript:;\" ng-click=\"gq(customer)\">挂起</button> </td> </tr> </tbody> </table> <div class=\"u-paginator\"> <uib-pagination total-items=\"paginator.total\" items-per-page=\"paginator.perPage\" boundary-links=\"true\" ng-model=\"paginator.currentPage\" previous-text=\"{{::paginator.previousText}}\" next-text=\"{{::paginator.nextText}}\" first-text=\"{{::paginator.firstText}}\" last-text=\"{{::paginator.lastText}}\" force-ellipses=\"true\" max-size=\"6\" num-pages=\"paginator.numPages\" ng-change=\"pageChanged()\"> </uib-pagination> <div class=\"u-pages\"> <span>{{paginator.currentPage}}</span> / <span>{{paginator.numPages}}</span> &nbsp; <span>合计:{{paginator.total}}</span> </div> <form name=\"paginatorForm\" novalidate> <div class=\"input-group\"> <input type=\"number\" name=\"paginatorInput\" ng-model=\"currentPage\" required min=\"1\" max=\"{{paginator.numPages}}\" ng class=\"form-control\"> <span class=\"input-group-btn\"> <button class=\"btn btn-default\" type=\"button\" ng-disabled=\"!paginatorForm.$valid\" ng-click=\"setCurrentPage()\">Go!</button> </span> </div> </form> </div> </div>"
  );
  ...
  $templateCache.put('views/finance_manageContract.html',
    "<div class=\"recharge\"> <div class=\"well well-sm u-search clearfix\"> <form class=\"form-inline\" style=\"position:relative;padding-right:150px\"> <div class=\"form-group\" style=\"margin: 5px 10px 5px 10px\"> <label>所属公司</label>： <select class=\"form-control\" ng-model=\"search.subsidairy\"> <option value=\"\" ng-selected=\"true\">请选择</option> <option ng-repeat=\"item in companys\" ng-value=\"item.SubsidiaryId\">{{item.CompanyName}}</option> </select> </div> <div class=\"form-group\" style=\"margin: 5px 10px 5px 10px\"> <label>合同编号</label>： <input type=\"text\" ng-model=\"search.contractNo\" class=\"form-control\"> </div> <div class=\"form-group\" style=\"margin: 5px 10px 5px 10px\"> <label>甲方</label>： <input type=\"text\" ng-model=\"search.companyname\" class=\"form-control\"> </div> <div class=\"form-group\" style=\"margin: 5px 10px 5px 10px\"> <label>联系人</label>： <input type=\"text\" ng-model=\"search.contact\" class=\"form-control\"> </div> <div class=\"form-group\" style=\"margin: 5px 10px 5px 10px\"> <label>销售人员</label>： <input type=\"text\" ng-model=\"search.saleName\" class=\"form-control\"> </div> <div class=\"form-group\" style=\"margin: 5px 10px 5px 10px\"> <label>合同状态</label>： <select ng-model=\"search.contractStatus\" class=\"form-control\"> <option value=\"0\">请选择</option> <option ng-repeat=\"(key,value) in contractStatus\" ng-value=\"key\">{{value}}</option> </select> </div> <div class=\"form-group\" style=\"margin: 5px 10px 5px 10px\"> <label>合同类型</label>： <select ng-model=\"search.contractType\" class=\"form-control\"> <option value=\"0\">请选择</option> <option ng-repeat=\"(key,value) in contractType\" ng-value=\"key\">{{value}}</option> </select> </div> <div class=\"form-group\" style=\"margin: 5px 10px 5px 10px\"> <label>财务审核状态</label>： <select ng-model=\"search.FinancialAudit\" class=\"form-control\"> <option value=\"0\">请选择</option> <option ng-repeat=\"(key,value) in accountStatus\" ng-value=\"key\">{{value}}</option> </select> </div> <div class=\"form-group\" style=\"margin: 5px 10px 5px 10px;min-width:300px\"> <lable style=\"margin-bottom: 5px;font-weight: bold;float:left;line-height:34px;height:34px\">合同签订日期：</lable> <div class=\"row\" style=\"float:left\"> <div class=\"col-md-6\"> <p class=\"input-group\" style=\"margin:0\"> <input type=\"text\" class=\"form-control\" uib-datepicker-popup=\"{{format}}\" ng-model=\"dt1\" is-open=\"popup1.opened\" datepicker-options=\"dateOptions\" close-text=\"Close\" alt-input-formats=\"altInputFormats\" ng-change=\"changeDate()\" id=\"date1\" style=\"padding:6px\"> <span class=\"input-group-btn\"> <button type=\"button\" class=\"btn btn-default\" ng-click=\"open1()\"><i class=\"glyphicon glyphicon-calendar\"></i></button> </span> </p> </div> <div class=\"col-md-6\"> <p class=\"input-group\" style=\"margin:0\"> <input type=\"text\" class=\"form-control\" uib-datepicker-popup ng-model=\"dt2\" is-open=\"popup2.opened\" datepicker-options=\"dateOptions\" close-text=\"Close\" ng-change=\"changeDate()\" id=\"date2\" style=\"padding:6px\"> <span class=\"input-group-btn\"> <button type=\"button\" class=\"btn btn-default\" ng-click=\"open2()\"><i class=\"glyphicon glyphicon-calendar\"></i></button> </span> </p> </div> </div> </div> <button type=\"button\" class=\"btn btn-primary\" ng-click=\"searchFn()\" style=\"position:absolute;right:20px;top:5px\">搜索</button> <button class=\"btn btn-primary pull-right\" ng-click=\"toExcel()\" style=\"position:absolute;right:20px;top:50px\">导出</button> </form> </div> <div js-height> <table class=\"table table-striped table-hover table-bordered\" id=\"dataTable\"> <thead> <tr class=\"info\"> <th>选择</th> <th>所属公司</th> <th>合同编号</th> <th>甲方</th> <th>联系人</th> <th>销售人员</th> <th>合同类型</th> <th>合同状态</th> <th>合同签订日期</th> <th>合同总金额</th> <th>财务审核</th> <th>最后操作人</th> <th>最后修改日期</th> <th>操作</th> </tr> </thead> <tbody> <tr ng-repeat=\"contract in contracts\"> <td><input type=\"checkbox\" ng-model=\"contract.selected\" ng-disabled=\"contract.FinancialAudit != 1 \" style=\"vertical-align: top; width:16px;height:16px\"></td> <td style=\"text-align:center\">{{contract.SubsidiaryName}}</td> <td style=\"text-align:center\">{{contract.ContractNo}}</td> <td style=\"text-align:center\">{{contract.CompanyName}}</td> <td style=\"text-align:center\">{{contract.Connector}}</td> <td style=\"text-align:center\">{{contract.SaleName}}</td> <td style=\"text-align:center\">{{contract.OrderType | ContractType}}</td> <td style=\"text-align:center\">{{contract.OrderStatus | Contractstatus}}</td> <td style=\"text-align:center\">{{contract.ContractDate | formateDate}}</td> <td style=\"text-align:center\">{{contract.Amount}}</td> <td style=\"text-align:center\">{{contract.FinancialAudit | FinanceStatus}}</td> <td style=\"text-align:center\">{{contract.ModifyUserName}}</td> <td style=\"text-align:center\">{{contract.ModifyDate}}</td> <td class=\"table-opt\"> <button class=\"btn btn-link\" href=\"javascript:;\" ng-click=\"detail(contract)\">查看</button> <button class=\"btn btn-link\" ng-disabled=\"contract.FinancialAudit != 1\" ng-click=\"check(contract)\">审核</button> <button class=\"btn btn-link\" ng-disabled=\"contract.FinancialAudit != 1\" ng-click=\"refuse(contract)\">驳回</button> </td> </tr> </tbody> </table> <div style=\"padding-left:10px;line-height: 23px\"> <lable> <input type=\"checkbox\" style=\"vertical-align: top; width:16px;height:16px\" ng-model=\"forwards.isSelectAll\" ng-change=\"selectAll()\"> 全选 </lable> <button style=\"padding:0 5px;border: 1px solid #ccc;background: #eee\" class=\"btn\" type=\"button\" ng-click=\"checkAll()\">批量审核</button> </div> </div> <div class=\"u-paginator\"> <uib-pagination total-items=\"paginator.total\" items-per-page=\"paginator.perPage\" boundary-links=\"true\" ng-model=\"paginator.currentPage\" previous-text=\"{{::paginator.previousText}}\" next-text=\"{{::paginator.nextText}}\" first-text=\"{{::paginator.firstText}}\" last-text=\"{{::paginator.lastText}}\" force-ellipses=\"true\" max-size=\"6\" num-pages=\"paginator.numPages\" ng-change=\"pageChanged()\"> </uib-pagination> <div class=\"u-pages\"> <span>{{paginator.currentPage}}</span> / <span>{{paginator.numPages}}</span> &nbsp; <span>合计:{{paginator.total}}</span> </div> <form name=\"paginatorForm\" novalidate> <div class=\"input-group\"> <input type=\"number\" name=\"paginatorInput\" ng-model=\"currentPage\" required min=\"1\" max=\"{{paginator.numPages}}\" ng class=\"form-control\"> <span class=\"input-group-btn\"> <button class=\"btn btn-default\" type=\"button\" ng-disabled=\"!paginatorForm.$valid\" ng-click=\"setCurrentPage()\">Go!</button> </span> </div> </form> </div> </div>"
  );
  ...
  $templateCache.put('views/order_outworker_add.html',
    "<div class=\"modal-header\"> <span class=\"close\" ng-click=\"cancel()\">&times;</span> <h3 class=\"modal-title\">添加外勤任务</h3> </div> <div class=\"modal-body\"> <form name=\"cusform\" novalidate> <div class=\"well well-sm\" style=\"min-height: 200px\"> <uib-accordion close-others=\"true\"> <div uib-accordion-group class=\"panel-default\" close-others=\"true\" is-open=\"open1\" is-disabled=\"tbIsDisable()\"> <uib-accordion-heading> 通办任务 <i class=\"pull-right glyphicon\" ng-class=\"{'glyphicon-chevron-down': accordion.open1, 'glyphicon-chevron-right': !accordion.open1}\"></i> </uib-accordion-heading> <div ng-repeat=\"task in tasksArr\" style=\"width:100%;min-height:50px;margin:2px;background:#eee;max-height:280px\"> <p> <lable style=\"width:100%;font-size:16px\"><input type=\"checkbox\" name=\"commonTask\" ng-model=\"task.checked\" ng-change=\"checkTB(task)\" ng-value=\"true\">&nbsp;&nbsp;&nbsp;{{task.CommonTaskName}}</lable> </p> <ul style=\"padding-left:20px;min-height:40px;display: inline-block\"> <li style=\"float:left;margin-right:12px;height:18px;list-style:none\" ng-repeat=\"item in task.TaskList\"> {{$index+1}}.{{item.TaskName}} </li> </ul> </div> </div> <div uib-accordion-group class=\"panel-default\" close-others=\"true\" is-open=\"open2\" is-disabled=\"otherIsDisable()\"> <uib-accordion-heading> 其他 <i class=\"pull-right glyphicon\" ng-class=\"{'glyphicon-chevron-down': accordion.open2, 'glyphicon-chevron-right': !accordion.open2}\"></i> </uib-accordion-heading> <uib-accordion close-others=\"false\"> <div uib-accordion-group ng-repeat=\"tg in tasks\" class=\"panel-default\" is-open=\"true\"> <uib-accordion-heading> {{::tg.Name}}任务 </uib-accordion-heading> <label style=\"margin:6px 30px 6px 0;font-size:16px;display:inline-block\" ng-repeat=\"task in tg.list\"><input type=\"checkbox\" ng-model=\"task.selected\">{{::task.TaskName}}</label> </div> </uib-accordion> </div> </uib-accordion> </div> <div class=\"well well-sm u-search form-inline clearfix\"> <div class=\"form-group col-md-12\"> <label class=\"required\">选择公司</label>： <input flex ng-value=\"postData.Customer.CompanyName || postData.Customer.Connector\" required ng-disabled=\"cus_selected\" aria-label=\"CompanyName\" ng-click=\"selectCustomer()\" readonly class=\"form-control\"> </div> <div class=\"form-group col-md-12\" style=\"margin-top:20px\"> <label class=\"required\">选择区域</label>： <select ng-model=\"areaSele\" class=\"form-control\" required value=\"0\" ng-change=\"change()\" required> <option value=\"\" ng-selected=\"true\">请选择</option> <option ng-repeat=\"area in areaArr\" ng-value=\"area.AreaCode\">{{area.AreaName}}</option> </select> </div> <div class=\"form-group col-md-12\" style=\"margin: 12px 0\"> <label>备注</label>： <textarea name=\"\" flex ng-model=\"postData.Remark\" maxlength=\"400\" style=\"width: 100%;height: 100px\"></textarea> </div> </div> </form> </div> <div class=\"modal-footer\"> <button class=\"btn btn-primary\" type=\"button\" ng-disabled=\"cusform.$invalid\" ng-click=\"ok($event)\">保存</button> <button class=\"btn btn-warning\" type=\"button\" ng-click=\"cancel()\">关闭</button> </div>  "
  );


  $templateCache.put('views/order_outworker_detail.html',
    "<div class=\"modal-header\"> <span class=\"close\" ng-click=\"cancel()\">&times;</span> <h3 class=\"modal-title\">任务详情</h3> <div ng-if=\"ifSub\" style=\"display:inline;float:right\"> <button class=\"btn btn-warning\" type=\"button\" ng-click=\"sub()\" ng-disabled=\"isSub(customer)\">提交会计</button> <button uib-popover=\"{{dynamicPopover.content}}\" popover-title=\"{{dynamicPopover.title}}\" type=\"button\" class=\"btn btn-default\"> <span class=\"glyphicon glyphicon-envelope\"></span> </button> </div> </div> <div class=\"modal-body\"> <table class=\"table table-striped table-hover table-bordered\"> <thead> <tr class=\"info\"> <!--<th style=\"width: 40px;\" ng-show=\"batIsOpen\"></th>--> <th ng-show=\"user.Category ==2 || user.Category==8\">选择</th> <th>序列ID</th> <th>子任务名称</th> <th>当前外勤人员</th> <th>开始时间</th> <th>完成时间</th> <th style=\"width:70px\">状态</th> <th style=\"width:230px\">操作</th> </tr> </thead> <tbody> <tr ng-repeat=\"customer in customers track by customer.TaskId\"> <td ng-show=\"user.Category ==2 || user.Category==8\"><input type=\"checkbox\" ng-model=\"customer.selected\" ng-disabled=\"customer.Status>3\" style=\"vertical-align: top; width:16px;height:16px\"></td> <td>{{customer.Id}}</td> <td>{{customer.TaskName}}</td> <td>{{customer.OutWorkerName}}</td> <td>{{customer.StartTime|tDateTime}}</td> <td>{{customer.EndTime|tDateTime}}</td> <td>{{customer.Status|outWorkStatus}}</td> <td><button class=\"btn btn-link\" ng-click=\"editCompany(customer)\" ng-if=\"user.Category ==2 || user.Category==8 || customer.OutWorkerId == user.UserId\" ng-disabled=\"(user.Category ==2 || user.Category==8)? false :customer.Status != 3\">编辑</button> <button class=\"btn btn-link\" ng-click=\"statusChange(customer,3,'确认资料')\" ng-if=\"user.Category ==2 || user.Category==8 || customer.OutWorkerId == user.UserId\" ng-disabled=\"customer.Status!==2\">确认资料</button> <button class=\"btn btn-link\" ng-click=\"statusChange(customer,5,'完成')\" ng-if=\"user.Category ==2 || user.Category==8 || customer.OutWorkerId == user.UserId\" ng-disabled=\"customer.Status!==3\">完成</button> <button class=\"btn btn-link\" ng-click=\"forward(customer)\" ng-if=\"user.Category ==2 || user.Category==8\" ng-disabled=\"customer.Status>3\">转接任务</button> <button class=\"btn btn-link\" ng-click=\"statusChange(customer,4,'取消任务')\" ng-if=\"user.Category ==2 || user.Category==8\" ng-disabled=\"customer.Status>3\">取消</button></td> </tr> </tbody> </table> <lable ng-if=\"user.Category ==2 || user.Category==8\"><input type=\"checkbox\" style=\"vertical-align: top; width:16px;height:16px\" ng-model=\"forwards.isSelectAll\" ng-change=\"selectAll()\">全选</lable> <select ng-change=\"forwardAll()\" ng-model=\"forwards.forwardUserId\" ng-if=\"user.Category ==2 || user.Category==8\"> <option selected value=\"0\">任务批量分配</option> <option ng-repeat=\"ot in outworkers\" value=\"{{::ot.UserId}}\">{{::ot.RealName}}</option> </select> </div> <div class=\"modal-footer\"> <button class=\"btn btn-warning\" type=\"button\" ng-click=\"cancel()\">关闭</button> </div>"
  );


  $templateCache.put('views/order_outworker_detail_sub.html',
    "<div class=\"modal-header\"> <span class=\"close\" ng-click=\"close()\">&times;</span> <h3 class=\"modal-title\">外勤提交会计</h3> </div> <div class=\"modal-body\"> <label for=\"\" style=\"display:block\">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <input type=\"radio\" name=\"art\" value=\"0\" ng-model=\"partT\" ng-click=\"_partT = false\"><span style=\"display:inline-block;width:80%;height:80px;line-height:80px;padding-left:20px;border:1px dashed black\">资料齐全，提交会计审核</span> </label> <label for=\"\" style=\"display:block\">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <input type=\"radio\" name=\"art\" value=\"\" ng-checked=\"_partT\" ng-disabled=\"part\" style=\"position:relative;top:-20px\"> <span style=\"display:inline-block;width:80%;height:85px;line-height:40px;padding-left:20px;border:1px dashed black;position: relative;\n" +
    "    left: -4px\">部分税务报道<br> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <label for=\"\"> <input type=\"radio\" name=\"art_sub\" value=\"1\" ng-model=\"partT\" ng-click=\"_partT = true\" ng-disabled=\"part\">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;国税报道完毕 </label> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <label for=\"\"> <input type=\"radio\" name=\"art_sub\" value=\"2\" ng-model=\"partT\" ng-click=\"_partT = true\" ng-disabled=\"part\">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;地税报道完毕 </label> </span> </label> </div> <div class=\"modal-footer\"> <button class=\"btn btn-warning\" type=\"button\" ng-click=\"sub()\" ng-disabled=\"dis()\">提交</button> </div>"
  );
  ...
  $templateCache.put('views/outwork_manage.html',
    "<div class=\"recharge\"> <div class=\"well well-sm u-search\" style=\"min-height: 146px\"> <form class=\"form-inline\" style=\"position:relative;padding-right:150px\"> <div class=\"form-group\" style=\"margin: 5px 10px 5px 10px;float:none\"> <label>序列ID</label>： <input type=\"text\" ng-model=\"search.Id\" class=\"form-control\"> </div> <div class=\"form-group\" style=\"margin: 5px 10px 5px 10px;float:none\"> <label>公司名称</label>： <input type=\"text\" ng-model=\"search.companyname\" class=\"form-control\"> </div> <div class=\"form-group\" style=\"margin: 5px 10px 5px 10px;float:none\"> <label>公司联系人</label>： <input type=\"text\" ng-model=\"search.connector\" class=\"form-control\"> </div> <div class=\"form-group\" style=\"margin: 5px 10px 5px 10px;float:none\"> <label>任务名称</label>： <select ng-model=\"search.taskname\" class=\"form-control\"> <option value=\"\" ng-selected=\"true\">请选择</option> <option ng-repeat=\"task in Tasks\" ng-value=\"task.CommonTaskId\">{{task.CommonTaskName}}</option> <option ng-value=\"0\">其他</option> <!--需要改数据--> </select> </div> <div class=\"form-group\" style=\"margin: 5px 10px 5px 10px;float:none\"> <label>当前子任务</label>： <select ng-model=\"search.childtaskname\" class=\"form-control\"> <option value=\"\" ng-selected=\"true\">请选择</option> <option ng-repeat=\"outT in outTasks\" ng-value=\"outT.Id\">{{outT.TaskName}}</option> </select> </div> <div class=\"form-group\" style=\"margin: 5px 10px 5px 10px;float:none\"> <label>外勤人员</label>： <select ng-model=\"search.currOutworker\" class=\"form-control\"> <option value=\"\" ng-selected=\"true\">请选择</option> <option ng-repeat=\"ot in outworkers\" ng-value=\"{{ot.UserId}}\">{{ot.RealName}}</option> <!--需要改数据--> </select> </div> <div class=\"form-group\" style=\"margin: 5px 10px 5px 10px;float:none\"> <label>公司状态</label>： <select ng-model=\"search.servicestatus\" class=\"form-control\"> <option value=\"\" ng-selected=\"true\">请选择</option> <option value=\"1\">等待分配</option> <option value=\"2\">未开始</option> <option value=\"3\">外勤服务</option> <option value=\"4\">外勤会计服务</option> <option value=\"5\">会计服务</option> <option value=\"6\">挂起</option> <option value=\"7\">结束</option> <!--需要改数据--> </select> </div> <!--日期--> <div class=\"form-group\" style=\"margin: 5px 10px 5px 10px;min-width:300px;float:none\"> <lable style=\"margin-bottom: 5px;font-weight: bold;float:left;line-height:34px;height:34px\">任务提交日期：</lable> <div class=\"row\" style=\"float:left\"> <div class=\"col-md-6\"> <p class=\"input-group\" style=\"margin:0\"> <input type=\"text\" class=\"form-control\" uib-datepicker-popup=\"{{format}}\" ng-model=\"dt1\" is-open=\"popup1.opened\" datepicker-options=\"dateOptions\" close-text=\"Close\" alt-input-formats=\"altInputFormats\" ng-change=\"changeDate()\" id=\"date1\" style=\"padding:6px\"> <span class=\"input-group-btn\"> <button type=\"button\" class=\"btn btn-default\" ng-click=\"open1()\"><i class=\"glyphicon glyphicon-calendar\"></i></button> </span> </p> </div> <div class=\"col-md-6\"> <p class=\"input-group\" style=\"margin:0\"> <input type=\"text\" class=\"form-control\" uib-datepicker-popup ng-model=\"dt2\" is-open=\"popup2.opened\" datepicker-options=\"dateOptions\" close-text=\"Close\" ng-change=\"changeDate()\" id=\"date2\" style=\"padding:6px\"> <span class=\"input-group-btn\"> <button type=\"button\" class=\"btn btn-default\" ng-click=\"open2()\"><i class=\"glyphicon glyphicon-calendar\"></i></button> </span> </p> </div> </div> </div> <div class=\"form-group\" style=\"margin: 5px 10px 5px 10px;float:none\"> <label>所属区（县）</label>： <select ng-model=\"search.areacode\" class=\"form-control\"> <option value=\"\" ng-selected=\"true\">请选择</option> <option ng-repeat=\"area in areas\" ng-value=\"area.AreaCode\">{{area.AreaName}}</option> <!--需要改数据--> </select> </div> <div class=\"form-group\" style=\"margin: 5px 10px 5px 10px;float:none\"> <label>销售人员</label>： <select ng-model=\"search.salesId\" class=\"form-control\"> <option value=\"0\" ng-selected=\"true\">请选择</option> <option ng-repeat=\"u in users\" ng-value=\"u.UserId\">{{u.RealName}}</option> <!--需要改--> </select> </div> <div class=\"form-group\" style=\"margin: 5px 10px 5px 10px;float:none\"> <label>任务状态</label>： <select ng-model=\"search.taskstatus\" class=\"form-control\"> <option value=\"\" ng-selected=\"true\">请选择</option> <option ng-repeat=\"(key,value) in taskStatus\" ng-value=\"key\">{{value}}</option> <!--需要改--> </select> </div> <!--<div class=\"form-group\">\n" +
    "                <a href=\"\">导出Excel表格</a>\n" +
    "            </div>--> <button type=\"button\" class=\"btn btn-primary\" ng-click=\"searchFn()\" style=\"position:absolute;right:20px;top:5px;width:110px\">搜索</button> <button class=\"btn btn-primary pull-right\" ng-click=\"open()\" style=\"position:absolute;right:20px;top:50px;width:110px\" ng-if=\"user.Category ==2 || user.Category==8 || user.Category==5\">新增</button> </form> </div> <table class=\"table table-striped table-hover table-bordered\"> <thead> <tr class=\"info\"> <th>序列ID</th> <th>公司名称</th> <!-- <th>任务名称</th> --> <th>所属区域</th> <th>公司联系人</th> <th>销售人员</th> <!----> <th>公司状态</th> <th>审核状态</th> <th>会计审核</th> <th>部分报税</th> <th>任务名称</th> <th>总任务状态</th> <!----> <th>当前子任务</th> <th>当前外勤人员</th> <th>提交任务时间</th> <th>子任务状态</th> <th>操作</th> </tr> </thead> <tbody> <tr ng-repeat=\"customer in customers\"> <td style=\"text-align:center\">{{customer.Id}}</td> <!-- <td style=\"text-align:center\">{{customer.MainTaskName}}</td> --> <td style=\"text-align:center\">{{customer.CompanyName}}</td> <td style=\"text-align:center\">{{customer.AreaName}}</td> <td style=\"text-align:center\">{{customer.Connector}}</td> <td style=\"text-align:center\">{{customer.SalesName}}</td> <!----> <td style=\"text-align:center\">{{customer.ServiceStatus|ComStatus}}</td> <td style=\"text-align:center\">{{customer.OutWorkerStatus|ACauditStatus}}</td> <td style=\"text-align:center\">{{customer.AccountantStatus|ACauditStatus}}</td> <td style=\"text-align:center\">{{customer.PartTax|partTax}}</td> <td style=\"text-align:center\">{{customer.MainTaskName}}</td> <td style=\"text-align:center\">{{customer.MainTaskStatus|mainTaskStatus}}</td> <!----> <td style=\"text-align:center\">{{customer.childTaskName}}</td> <td style=\"text-align:center\">{{customer.OutWorkerName}}</td> <td style=\"text-align:center\">{{customer.SubmitTaskTime|tDate}}</td> <td style=\"text-align:center\">{{customer.Status|outWorkStatus}}</td> <td class=\"table-opt\" style=\"text-align:center;padding:4px\"> <button class=\"btn btn-link\" href=\"javascript:;\" ng-click=\"detail(customer)\" style=\"padding:1px\">查看</button> <button class=\"btn btn-link\" href=\"javascript:;\" ng-click=\"check(customer)\" style=\"padding:1px\" ng-disabled=\"ifCheck(customer)\">审核</button> <button class=\"btn btn-link\" href=\"javascript:;\" ng-click=\"reject(customer)\" style=\"padding:1px\" ng-disabled=\"ifCheck(customer)\">驳回</button> <button class=\"btn btn-link\" href=\"javascript:;\" ng-click=\"cancel(customer)\" style=\"padding:1px\">取消</button> <button class=\"btn btn-link\" href=\"javascript:;\" ng-click=\"sub(customer)\" style=\"padding:1px\" ng-disabled=\"isSub(customer)\">提交</button> <!--   --> </td></tr> </tbody> </table> <div class=\"u-paginator\"> <uib-pagination total-items=\"paginator.total\" items-per-page=\"paginator.perPage\" boundary-links=\"true\" ng-model=\"paginator.currentPage\" previous-text=\"{{::paginator.previousText}}\" next-text=\"{{::paginator.nextText}}\" first-text=\"{{::paginator.firstText}}\" last-text=\"{{::paginator.lastText}}\" force-ellipses=\"true\" max-size=\"6\" num-pages=\"paginator.numPages\" ng-change=\"pageChanged()\"> </uib-pagination> <div class=\"u-pages\"> <span>{{paginator.currentPage}}</span> / <span>{{paginator.numPages}}</span> &nbsp; <span>合计:{{paginator.total}}</span> </div> <form name=\"paginatorForm\" novalidate> <div class=\"input-group\"> <input type=\"number\" name=\"paginatorInput\" ng-model=\"currentPage\" required min=\"1\" max=\"{{paginator.numPages}}\" ng class=\"form-control\"> <span class=\"input-group-btn\"> <button class=\"btn btn-default\" type=\"button\" ng-disabled=\"!paginatorForm.$valid\" ng-click=\"setCurrentPage()\">Go!</button> </span> </div> </form> </div> </div>"
  );
  ...
  $templateCache.put('views/signed_detail.html',
    "<div class=\"modal-header\"> <span class=\"close\" ng-click=\"cancel()\">&times;</span> <h3 class=\"modal-title\">签约客户管理>查看</h3> </div> <div class=\"modal-body\"> <div class=\"content-header clearfix\"> <form name=\"contractBelongform\" novalidate class=\"con-header-left fl\"> <div class=\"form-group ml5\"> <label>序列ID</label>： <input flex ng-model=\"postDetail.Id\" required readonly class=\"input-border\"> </div> <div class=\"form-group ml5\"> <label>公司名称</label>： <input flex ng-model=\"postDetail.CompanyName\" required readonly class=\"input-border\" style=\"width:200px\"> </div> <div class=\"form-group ml5\"> <label>当前合同编号</label>： <input flex ng-model=\"postDetail.contractId\" required readonly class=\"input-border\"> </div> <div class=\"form-group ml5\"> <label>当前服务日期</label>： <input flex ng-model=\"postDetail.serviceDate\" required readonly class=\"input-border\"> </div> <div class=\"form-group ml5\"> <label>公司状态</label>： <input flex value=\"{{postDetail.companystatus | CustomerCompanyStatus}}\" required readonly class=\"input-border\" style=\"width:100px\"> </div> <div class=\"form-group ml5\"> <label>外勤审核状态</label>： <input flex value=\"{{postDetail.outworkstatus | CustomerOutworkStatus}}\" required readonly class=\"input-border\" style=\"width:100px\"> </div> <div class=\"form-group ml5\"> <label>会计审核状态</label>： <input flex value=\"{{postDetail.accountstatus | CustomeraccountStatus}}\" required readonly class=\"input-border\" style=\"width:100px\"> </div> </form> <div class=\"con-header-right fr\"> <button class=\"btn btn-primary mt10\" type=\"button\" ng-click=\"submitAccount()\">审核提交会计</button> <button class=\"btn btn-primary\" type=\"button\" ng-click=\"submitOutwork()\">审核提交外勤</button> </div> </div> <div class=\"content-tab\"> <form name=\"outerForm\" class=\"tab-form-demo\"> <uib-tabset active=\"activeForm\"> <uib-tab index=\"0\" heading=\"公司信息\"> <div class=\"customer-info clearfix\"> <div class=\"title\" style=\"margin-bottom: 10px\"> <div class=\"fl belong-left\"> <span>客户基本信息</span> </div> <div class=\"fr belong-right\"> <button ng-if=\"isEdit\" class=\"btn\" type=\"button\" ng-click=\"edit()\">编辑</button> <button ng-if=\"!isEdit\" class=\"btn\" type=\"button\" ng-click=\"save()\">完成</button> </div> </div> <form name=\"customerform\" novalidate class=\"clearfix\"> <div class=\"form-group ml5\"> <label class=\"required\">公司名称</label>： <input flex ng-model=\"postDetail.CompanyName\" required ng-disabled=\"isEdit\" class=\"input-border\" style=\"width:250px\"> </div> <div class=\"form-group ml5\"> <label class=\"required\">公司联系人</label>： <input flex ng-model=\"postDetail.Connector\" required ng-disabled=\"isEdit\" class=\"input-border\" style=\"width:100px\"> </div> <div class=\"form-group ml5\"> <label class=\"required\">联系人电话</label>： <input flex ng-model=\"postDetail.Mobile\" required ng-disabled=\"isEdit\" class=\"input-border\"> </div> <div class=\"form-group ml5\"> <label class=\"required\">所属销售</label>： <input flex ng-model=\"postDetail.SaleName\" required readonly class=\"input-border\"> </div> <div class=\"form-group ml5\"> <label>企业类型</label>： <span class=\"input-border companyTypeStyle\">{{postDetail.companyType | CompanyType}}</span> </div> <div class=\"form-group ml5\"> <label>所属行业</label>： <select ng-model=\"postDetail.industryId\" class=\"input-border\" ng-disabled=\"isEdit\" style=\"width:100px\"> <option value=\"\">请选择</option> <option ng-repeat=\"item in industries\" value=\"item.industryId\" ng-selected=\"postDetail.industryId == item.industryId\">{{item.industryName}}</option> </select> </div> <div class=\"form-group ml5\"> <label>所属区域</label>： <select ng-model=\"postDetail.areaId\" class=\"input-border\" ng-disabled=\"isEdit\" style=\"width:100px\"> <option value=\"\">请选择</option> <option ng-repeat=\"item in areas\" value=\"item.areaId\" ng-selected=\"postDetail.areaId == item.areaId\">{{item.areaName}}</option> </select> </div> <div class=\"form-group ml5\"> <label>公司地址</label>： <input flex ng-model=\"postDetail.address\" required ng-disabled=\"isEdit\" class=\"input-border\" style=\"width:300px\"> </div> </form> </div> <div class=\"customer-info clearfix\"> <div class=\"title\" style=\"margin-bottom: 10px\">营业执照信息</div> <form name=\"companyform\" novalidate class=\"clearfix\"> <div class=\"form-group ml5\"> <label>法人姓名</label>： <input flex ng-model=\"postDetail.Name\" required ng-disabled=\"isEdit\" class=\"input-border\"> </div> <div class=\"form-group ml5 flex-style\"> <label>法人身份证</label>： <div style=\"display: inline-block\"> <img ng-src=\"{{postDetail.cardImgUrl1}}\" ng-if=\"postDetail.cardImgUrl1\" alt=\"\" pic-view class=\"contract-img-style\"> <div class=\"uploader-outter\"> <input class=\"uploader-img\" type=\"file\" nv-file-select=\"\" uploader=\"uploader1\" ng-disabled=\"isEdit\"> </div> <div class=\"progress\" style=\"width:80%;margin-left:90px\" ng-if=\"uploader1.isUploading\"> <div class=\"progress-bar\" role=\"progressbar\" ng-style=\"{ 'width': uploader1.progress + '%' }\" style=\"width: 100%\"></div> </div> </div> </div> <div class=\"form-group ml5\"> <label>注册号</label>： <input flex ng-model=\"postDetail.registrationId\" required ng-disabled=\"isEdit\" class=\"input-border\" style=\"width: 180px\"> </div> <div class=\"form-group ml5 flex-style\"> <label>营业执照</label>： <div style=\"display: inline-block\"> <img ng-src=\"{{postDetail.cardImgUrl2}}\" ng-if=\"postDetail.cardImgUrl2\" alt=\"\" pic-view class=\"contract-img-style\"> <div class=\"uploader-outter\"> <input class=\"uploader-img\" type=\"file\" nv-file-select=\"\" ng-disabled=\"isEdit\" uploader=\"uploader2\"> </div> <div class=\"progress\" style=\"width:80%;margin-left:90px\" ng-if=\"uploader2.isUploading\"> <div class=\"progress-bar\" role=\"progressbar\" ng-style=\"{ 'width': uploader2.progress + '%' }\" style=\"width: 100%\"></div> </div> </div> </div> <div class=\"form-group ml5\"> <label>国税登记号</label>： <input flex ng-model=\"postDetail.nationalTax\" required ng-disabled=\"isEdit\" class=\"input-border\" style=\"width: 180px\"> </div> <div class=\"form-group ml5\"> <label>地税登记号</label>： <input flex ng-model=\"postDetail.governmentTax\" required ng-disabled=\"isEdit\" class=\"input-border\" style=\"width: 180px\"> </div> <div class=\"form-group ml5\"> <label>注册资金</label>： <input flex ng-model=\"postDetail.registerAmount\" required ng-disabled=\"isEdit\" class=\"input-border\"> </div> <div class=\"form-group ml5\"> <lable style=\"margin-bottom: 5px;font-weight: bold;float:left;line-height:34px;height:34px\">营业期限：</lable> <div style=\"display: inline-block\"> <p class=\"input-group\" style=\"margin:0;width:150px\"> <input type=\"text\" class=\"form-control\" ng-click=\"open1()\" ng-model=\"postDetail.serviceDateStart\" uib-datepicker-popup=\"yyyy-MM-dd\" ng-model=\"dt\" is-open=\"popup1.opened\" datepicker-options=\"dateOptions\" close-text=\"关闭\" alt-input-formats=\"altInputFormats\" style=\"padding:6px\" ng-disabled=\"isEdit\"> <span class=\"input-group-btn\"> <button type=\"button\" class=\"btn btn-default\" ng-click=\"open1()\" ng-disabled=\"isEdit\"><i class=\"glyphicon glyphicon-calendar\"></i></button> </span> </p> </div> <div style=\"display: inline-block\"> <p class=\"input-group\" style=\"margin:0;width:150px\"> <input type=\"text\" class=\"form-control\" ng-click=\"open2()\" ng-model=\"postDetail.serviceDateEnd\" uib-datepicker-popup=\"yyyy-MM-dd\" ng-model=\"dt\" is-open=\"popup2.opened\" datepicker-options=\"dateOptions\" close-text=\"关闭\" alt-input-formats=\"altInputFormats\" style=\"padding:6px\" ng-disabled=\"isEdit || (postDetail.NoDeadLine == 1)\"> <span class=\"input-group-btn\"> <button type=\"button\" class=\"btn btn-default\" ng-click=\"open2()\" ng-disabled=\"isEdit || (postDetail.NoDeadLine == 1)\"><i class=\"glyphicon glyphicon-calendar\"></i></button> </span> </p> </div> <!-- 无限期 postDetail.NoDeadLine == 0 有期限 ==1无期限 同时前面结束时间不可点击   --> <div style=\"display: inline-block\"> <label ng-hide=\"isEdit && (postDetail.NoDeadLine == 0)\"> <input ng-hide=\"isEdit && (postDetail.NoDeadLine == 1)\" type=\"checkbox\" name=\"noLimitDate\" ng-readonly=\"isReadOnly\" value=\"1\" ng-click=\"postDetail.NoDeadLine = !postDetail.NoDeadLine; \" ng-checked=\"postDetail.NoDeadLine == 1\"> 无期限 </label> </div> </div> <div class=\"form-group\" style=\"margin: 5px 10px 5px 10px\"> <label>经营范围：</label> <textarea cols=\"120\" rows=\"5\" ng-model=\"postDetail.Remark\" ng-disabled=\"isEdit\"></textarea> </div> </form> </div> </uib-tab> <uib-tab index=\"1\" heading=\"合同信息\" select=\"refreshData2()\"> <div class=\"contract-info clearfix\"> <div class=\"title\" style=\"margin-bottom: 10px\">合同列表</div> <table class=\"table table-striped table-hover table-bordered\"> <thead> <tr class=\"info\"> <th>合同编号</th> <th>合同类型</th> <th>签订日期</th> <th>服务期限</th> <th>服务开始时间</th> <th>服务结束时间</th> <th>合同总金额</th> <th>操作</th> </tr> </thead> <tbody> <tr ng-repeat=\"contrac in contractTab\"> <td style=\"text-align:center\">{{contrac.contractId}}</td> <td style=\"text-align:center\">{{contrac.contractType | ContractType}}</td> <td style=\"text-align:center\">{{contrac.signTime}}</td> <td style=\"text-align:center\">{{contrac.service}}</td> <td style=\"text-align:center\">{{contrac.serviceDateStart}}</td> <td style=\"text-align:center\">{{contrac.serviceDateEnd}}</td> <td style=\"text-align:center\">{{contrac.contractAmount}}</td> <td class=\"table-opt\" style=\"text-align:center;width:200px\"> <button class=\"btn btn-link\" href=\"javascript:;\" ng-click=\"detailTab2(contrac)\">查看详情</button> <!-- 公司状态是挂起4 时候禁止点击终止合同 --> <button ng-disabled=\"isStop\" class=\"btn btn-link\" ng-disabled=\"contrac.OrderStatus == 4\" ng-click=\"stop(contrac)\">终止合同</button> </td> </tr> </tbody> </table> <!-- <div class=\"u-paginator\">\n" +
    ...
    "                </div>\n" +
    "              </form>\n" +
    "            </div> --> </div> </uib-tab> <uib-tab index=\"2\" heading=\"服务费及代收费\" select=\"refreshData3()\"> <div class=\"contract-info clearfix\"> <div class=\"title\" style=\"margin-bottom: 10px\">服务费及代收费列表</div> <table class=\"table table-striped table-hover table-bordered\"> <thead> <tr class=\"info\"> <th>合同编号</th> <th>服务期限</th> <th>合同总金额</th> <th>记账报税费用</th> <th>财务服务费</th> <th>外勤服务费</th> <th>代收费用</th> <th>操作</th> </tr> </thead> <tbody> <tr ng-repeat=\"contrac in contractTab3\"> <td style=\"text-align:center\">{{contrac.ContractNo}}</td> <td style=\"text-align:center\">{{contrac.service}}</td> <td style=\"text-align:center\">{{contrac.Amount | handdleAmount}}</td> <td style=\"text-align:center\">{{contrac.BookKeepFeed | handdleAmount}}</td> <td style=\"text-align:center\">{{contrac.FinanceServiceFeed | handdleAmount}}</td> <td style=\"text-align:center\">{{contrac.OutWorkServiceFeed | handdleAmount}}</td> <td style=\"text-align:center\">{{contrac.AgentFeed | handdleAmount}}</td> <td class=\"table-opt\" style=\"text-align:center;width:200px\"> <button class=\"btn btn-link\" href=\"javascript:;\" ng-click=\"detailTab3(contrac)\">查看详情</button> </td> </tr> </tbody> </table> <!-- <div class=\"u-paginator\">\n" +
    "              <uib-pagination total-items=\"tab2.paginator.total\" items-per-page=\"tab2.paginator.perPage\" boundary-links=\"true\" ng-model=\"tab2.paginator.currentPage\" previous-text=\"{{::tab2.paginator.previousText}}\" next-text=\"{{::tab2.paginator.nextText}}\" first-text=\"{{::tab2.paginator.firstText}}\"\n" +
    "                  last-text=\"{{::tab2.paginator.lastText}}\" force-ellipses=\"true\" max-size=\"6\" num-pages=\"tab2.paginator.numPages\" ng-change=\"tab2.pageChanged()\"> </uib-pagination>\n" +
    "              <div class=\"u-pages\"> <span>{{tab2.paginator.currentPage}}</span> / <span>{{tab2.paginator.numPages}}</span> &nbsp; <span>合计:{{tab2.paginator.total}}</span> </div>\n" +
    "              <form name=\"paginatorForm\" novalidate>\n" +
    ...
    "                </div>\n" +
    "              </form>\n" +
    "            </div> --> </div> </uib-tab> <uib-tab index=\"4\" heading=\"备注信息\" select=\"refreshData5()\"> <div class=\"contract-info clearfix\"> <div class=\"title\" style=\"margin-bottom: 10px\">备注信息</div> <table class=\"table table-striped table-hover table-bordered\"> <thead> <tr class=\"info\"> <th>序列号</th> <th>备注</th> <th>操作</th> <th>操作人</th> <th>操作时间</th> <th>操作</th> </tr> </thead> <tbody> <tr ng-repeat=\"contrac in contractTab5\"> <td style=\"text-align:center\">{{contrac.Id}}</td> <td style=\"text-align:center\">{{contrac.Content}}</td> <td style=\"text-align:center\">{{contrac.Operation | HanddleOperation}}</td> <td style=\"text-align:center\">{{contrac.OperatorId}}</td> <td style=\"text-align:center\">{{contrac.OperationTime}}</td> <td class=\"table-opt\" style=\"text-align:center;width:200px\"> <button class=\"btn btn-link\" href=\"javascript:;\" ng-click=\"detailTab5(contrac)\">查看详情</button> </td> </tr> </tbody> </table> <!-- <div class=\"u-paginator\">\n" +
    "              <uib-pagination total-items=\"tab2.paginator.total\" items-per-page=\"tab2.paginator.perPage\" boundary-links=\"true\" ng-model=\"tab2.paginator.currentPage\" previous-text=\"{{::tab2.paginator.previousText}}\" next-text=\"{{::tab2.paginator.nextText}}\" first-text=\"{{::tab2.paginator.firstText}}\"\n" +
    "                  last-text=\"{{::tab2.paginator.lastText}}\" force-ellipses=\"true\" max-size=\"6\" num-pages=\"tab2.paginator.numPages\" ng-change=\"tab2.pageChanged()\"> </uib-pagination>\n" +
    "              <div class=\"u-pages\"> <span>{{tab2.paginator.currentPage}}</span> / <span>{{tab2.paginator.numPages}}</span> &nbsp; <span>合计:{{tab2.paginator.total}}</span> </div>\n" +
    "              <form name=\"paginatorForm\" novalidate>\n" +
    ...
    "                </div>\n" +
    "              </form>\n" +
    "            </div> --> </div> </uib-tab> <uib-tab index=\"5\" heading=\"操作记录\" select=\"refreshData6()\"> <div class=\"contract-info clearfix\"> <div class=\"title\" style=\"margin-bottom: 10px\">操作记录</div> <table class=\"table table-striped table-hover table-bordered\"> <thead> <tr class=\"info\"> <th>序列号</th> <th>操作内容</th> <th>操作人</th> <th>操作时间</th> </tr> </thead> <tbody> <tr ng-repeat=\"contrac in contractTab6\"> <td style=\"text-align:center\">{{contrac.Id}}</td> <td style=\"text-align:center\">{{contrac.Content}}</td> <td style=\"text-align:center\">{{contrac.OperatorId}}</td> <td style=\"text-align:center\">{{contrac.OperationTime}}</td> </tr> </tbody> </table> <!-- <div class=\"u-paginator\">\n" +
