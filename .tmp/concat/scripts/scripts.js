...
}).filter('NewAgentStatus', function() { // 报税状态
...
}).filter('NewServiceStatus', function() { // 服务状态
...
}]).controller('SignedDetail1', ['$scope', '$http', '$uibModal', '$uibModalInstance', 'FileUploader','$filter', 'contractMsg', 'item', 'areas', 'customerId', 'user', function($scope, $http, $uibModal, $uibModalInstance, FileUploader, $filter, contract, item, areas, customerId, users) {
  var users = users.get()
  // console.log(users)
  console.log(contract, 'contract')
...
}]).controller('SubmitAccount', ['$scope', '$http', '$uibModal', '$uibModalInstance', 'postDataOut', 'isOnlyPartChoose', 'post', function($scope, $http, $uibModal, $uibModalInstance, postDataOut, isOnlyPartChoose, post) {
...
}]).controller("Signed_outworker_add", ['$scope', '$http', '$uibModal', '$uibModalInstance', 'isShow', 'postDataOut', 'user', function($scope, $http, $uibModal, $uibModalInstance, isShow, postDataOut, UserServe) {
  var user = UserServe.get()
...
angular.module('crmApp').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('views/accounting_manage.html',
    ...
    "                       --> <button class=\"btn btn-link\" href=\"javascript:;\" ng-click=\"autid(customer)\" ng-disabled=\"customer.AccountantStatus != 1\">审核</button> <button class=\"btn btn-link\" href=\"javascript:;\" ng-click=\"rejected(customer)\" ng-disabled=\"customer.AccountantStatus != 1\">驳回</button> </td> </tr> </tbody> </table> </div> <div class=\"u-paginator\"> <uib-pagination total-items=\"paginator.total\" items-per-page=\"paginator.perPage\" boundary-links=\"true\" ng-model=\"paginator.currentPage\" previous-text=\"{{::paginator.previousText}}\" next-text=\"{{::paginator.nextText}}\" first-text=\"{{::paginator.firstText}}\" last-text=\"{{::paginator.lastText}}\" force-ellipses=\"true\" max-size=\"6\" num-pages=\"paginator.numPages\" ng-change=\"pageChanged()\"> </uib-pagination> <div class=\"u-pages\"> <span>{{paginator.currentPage}}</span> / <span>{{paginator.numPages}}</span> &nbsp; <span>合计:{{paginator.total}}</span> </div> <form name=\"paginatorForm\" novalidate> <div class=\"input-group\"> <input type=\"number\" name=\"paginatorInput\" ng-model=\"currentPage\" required min=\"1\" max=\"{{paginator.numPages}}\" ng class=\"form-control\"> <span class=\"input-group-btn\"> <button class=\"btn btn-default\" type=\"button\" ng-disabled=\"!paginatorForm.$valid\" ng-click=\"setCurrentPage()\">Go!</button> </span> </div> </form> </div> </div>"
  );
